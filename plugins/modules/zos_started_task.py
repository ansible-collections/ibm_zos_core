...
options:
  ...
  asidx:
    description:
        - B(asid) is an unique address space identifier which gets assigned to each running started task.
...
#     type: str
  ...
  identifier_name:
    description:
        - B(identifier_name) is the name that identifies the task to be started. This name can be up to 8 characters long. Ensure that the first character is alphabetical.
  ...
  job_account:
    description:
        - B(job_account) specifies the accounting data in the JCL JOB statement for the started task. If the source JCL was a job and has already accounting data, the value that is specified on this parameter overrides the accounting data in the source JCL.
