...
options:
  ...
  purge:
    description:
      ...
      - If C(state=absent), the I(purge) attribute only applies to sequential, PDS, and PDSE data sets.
...
notes:
  ...
  - When C(state=absent), if C(purge=true) and C(scratch=false) or if C(purge=false) and C(scratch=false), a data set with an unreached
    expiration date is removed from the catalog but not physically deleted from the volume.
  - When C(state=absent), if C(purge=true) and C(scratch=true), a data set with an unreached expiration date is removed from the
    catalog and physically deleted from the volume.
"""
