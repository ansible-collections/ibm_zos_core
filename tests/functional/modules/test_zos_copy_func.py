...
def test_copy_file_to_empty_sequential_data_set(ansible_zos_module, src):
    ...
    try:
        hosts.all.zos_data_set(name=dest, type="PDSE", state="present")
...
def test_copy_file_to_non_empty_sequential_data_set(ansible_zos_module, src):
    ...
    try:
        hosts.all.zos_data_set(name=dest, type="PDSE", state="absent")
...
def test_copy_data_set_to_non_existing_member(ansible_zos_module, args):
    ...
    src = src_data_set if args["type"] == "PDSE" else "{0}(TEST)".format(src_data_set)
...
@pytest.mark.uss
@pytest.mark.pdse
@pytest.mark.parametrize("src_type", ["PDSE", "PDSE"])
...
@pytest.mark.seq
@pytest.mark.pdse
@pytest.mark.parametrize("src_type", ["PDSE", "PDSE", "PDSE"])
def test_copy_data_set_to_non_existing_pdse(ansible_zos_module, src_type):
    hosts = ansible_zos_module
    src_data_set = get_tmp_ds_name()
    src = src_data_set if src_type == "PDSE" else "{0}(TEST)".format(src_data_set)
    dest_data_set = get_tmp_ds_name()
    dest = "{0}(MEMBER)".format(dest_data_set)
...
def test_copy_multiple_data_set_members(ansible_zos_module):
    ...
    try:
        hosts.all.zos_data_set(name=src, type="PDSE")
        hosts.all.zos_data_set(name=dest, type="PDSE")
...
@pytest.mark.uss
@pytest.mark.pdse
@pytest.mark.parametrize("ds_type", ["PDSE", "PDSE"])
...
def test_copy_member_to_existing_seq_data_set(ansible_zos_module, args):
    ...
    try:
        hosts.all.zos_data_set(name=dest, type="PDSE", state="present", replace=True)
...
@pytest.mark.uss
@pytest.mark.pdse
@pytest.mark.parametrize("dest_type", ["PDSE", "PDSE"])
...
@pytest.mark.parametrize("args", [
    ...
    dict(type="PDSE", backup="USER.TEST.PDS.BACKUP"),
    dict(type="PDSE", backup=None),
