...
def test_copy_pds_loadlib_member_to_pds_loadlib_member(ansible_zos_module, is_created):
    ...
    hlq_size = 3
    cobol_src_pds = get_dataset(hosts, hlq_size)
    cobol_src_mem = get_dataset(hosts, hlq_size)
    src_lib = get_dataset(hosts, hlq_size)
    dest_lib = get_dataset(hosts, hlq_size)
    dest_lib_aliases = get_dataset(hosts, hlq_size)
    pgm_mem = "HELLO"
