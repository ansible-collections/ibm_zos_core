...
class CopyHandler(object):
    ...
    def copy_to_vsam(self, src, dest):
        ...
        rc, out, err = self._run_mvs_command("IDCAMS", repro_cmd, authorized=True)
        if rc != 0:
            self.fail_json(
                msg=(
    ...
    def _run_mvs_command(self, pgm, cmd, dd=None, authorized=False):
        ...
        return self.run_command(mvs_cmd.format(pgm, sysprint, sysin), data=cmd)
...
class PDSECopyHandler(CopyHandler):
    ...
    def copy_to_member(self, src, temp_path, conv_path, dest, copy_member=False):
        ...
        if is_uss_src:
            ...
            if rc != 0:
                self.fail_json(
                    msg="Unable to copy to data set member {0}".format(dest), rc=rc
                )
        return dest
        ...
        if members:
            try:
                for m in members.split("\n"):
                    ...
                    if rc != 0:
                        self.fail_json(
                            msg="Unable to delete data set member {0} from {1}".format(
                                m, data_set
                            ),
                            rc=rc,
                            out=out,
...
def backup_data(ds_name, ds_type, backup_file):
    ...
    try:
        ...
        module.fail_json(
            msg="Unable to back up destination {0}".format(ds_name), stderr=str(err)
        )
...
def run_module(module, arg_def):
    ...
    # ********************************************************************
    dest_name = extract_dsname(dest)
    src_name = extract_dsname(src) if src else None
    member_name = extract_member_name(src) if src_member else None
