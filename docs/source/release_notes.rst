...
- ``zos_copy``
   ...
   - Added a note to the documentation redirecting users to other modules to see how to recall migrated data sets before trying to copy them.
   - Adds new option `identical_gdg_copy` in the module.
   - This allows copying GDG generations from a source base to a destination base while preserving generation data set absolute names when the destination base does not exist prior to the copy.
   - Adds support of using alias names in src and dest parameters for PS, PDS and PDSE data sets.
   - Updated module documentation to reflect that binary destination data sets get allocated with a block size of 32720, instead of 32760.
...
- ``zos_lineinfile`` - The module would report a false negative when certain special characters where present in the `line` option. Fix now reports the successful operation.
