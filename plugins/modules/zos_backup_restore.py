...
options:
  ...
  terse:
    description:
      - When I(operation=backup), Uses AMATERSE to compress and pack the dump generated by ADRDSSU.
      - If I(operation=backup), & if I(dataset=False) Terse must be True.
    type: bool
    default: True
  sms_storage_class:
    description:
...
def to_dzip_args(**kwargs):
    ...
    # If dataset is False, terse must be True.
    if zoau_args.get("dataset") is False:
        zoau_args["terse"] = True
    elif kwargs.get("terse"):
