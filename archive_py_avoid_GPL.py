...
class Archive():
    def __init__(self, module):
        ...
        if self.format in ("terse", "xmit"):
            ...
            module.fail_json(
                    msg='expanded_paths: {0}'.format(self.expanded_paths)
                )
            if not self.paths:
                module.fail_json(
                    path=', '.join(paths),
