                ...
                if result[1] == "PO":
                    if pds_paths:
                        mls_rc, mls_out, mls_err = module.run_command(
                            "mls '{0}(*)'".format(result[0]), errors='backslashreplace'
                        )
                        if mls_rc == 2:
...
def _dgrep_wrapper(
    ...
    return AnsibleModuleHelper(argument_spec={}).run_command(dgrep_cmd, errors='backslashreplace')
...
def _vls_wrapper(pattern, details=False, verbose=False):
    ...
    return AnsibleModuleHelper(argument_spec={}).run_command(vls_cmd, errors='backslashreplace')
