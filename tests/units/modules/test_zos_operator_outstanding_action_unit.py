...
IMPORT_NAME = 'ansible_collections_ibm_zos_core.plugins.modules.zos_operator_outstanding_action'


# * Tests for zos_operator_outstanding_action
...
def test_zos_operator_outstanding_action_various_args(zos_import_mocker, args, expected):
    mocker, importer = zos_import_mocker
    zos_operator_outstanding_action = importer(IMPORT_NAME)
    passed = True
    mocker.patch('zoautil_py.OperatorCmd.execute',
                create=True, side_effect=[operator_result1,operator_result2])
    try:
        zos_operator_outstanding_action.find_required_request(args)
    except zos_operator_outstanding_action.Error:
        passed = False
...
def test__zos_operator_outstanding_action_failed_to_run_first_operator_command(zos_import_mocker):
    mocker, importer = zos_import_mocker
    passed = False
    ...
    try:
        zos_operator_outstanding_action.find_required_request(params)
    except zos_operator_outstanding_action.OperatorCmdError:
        passed = True
    assert passed == True

def test__zos_operator_outstanding_action_failed_to_run_second_operator_command(zos_import_mocker):
    mocker, importer = zos_import_mocker
    passed = False
    params = {'request_number_list': [ '001','010' ]}
    zos_operator_outstanding_action = importer(IMPORT_NAME)
...
def test__zos_operator_outstanding_action_failed_to_run_all_operator_command(zos_import_mocker):
    mocker, importer = zos_import_mocker
    passed = False
    params = {'request_number_list': ['001','010']}
    zos_operator_outstanding_action = importer(IMPORT_NAME)
...
def test__zos_operator_outstanding_action_missing_all_params(zos_import_mocker):
    mocker, importer = zos_import_mocker
    passed = True
    zos_operator_outstanding_action = importer(IMPORT_NAME)
    ...
    list = []
    try:
        list= zos_operator_outstanding_action.find_required_request(params)
    except zos_operator_outstanding_action.OperatorCmdError:
        pass
    assert len(list) != 0


def test__zos_operator_outstanding_action_empty_list(zos_import_mocker):
    mocker, importer = zos_import_mocker
    operator_result = {
    'rc': 0,
